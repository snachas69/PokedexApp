...
namespace PokedexApp.Service
{
	public class PokemonService : IService<Pokemon>
	{
		...
		public PokemonService(AppDbContext context, IConfiguration configuration)
		{
			_context = context;
		...
		{
			TypeService typeService = new TypeService(_context, _configuration);
			List<Models.Type> types = typeService.GetDataFromApi();

			List<Pokemon> pokemonList = new();
			HttpResponseMessage response = _client.GetAsync("pokemon?").Result;
			...
			if (response.IsSuccessStatusCode)
			{
				...
				foreach (JToken result in results)
				{
					...
					if (response.IsSuccessStatusCode)
					{
						...
						string name = (string)pokemonDetail["name"];
